...
Specify the name of the query fasta file:
```{r}
querySingleFasta <- "query.fasta"
```
Provide the path to the query. Default is to place the query file in the idFolder and don't change
anything in the following chunk.
```{r}
queryPath <- paste(sharedPath, idFolder, querySingleFasta, sep = "")
```

OR, to see the fasta files in the idFolder and choose a specific one interactively:
```{r}

...
Perform sets of queries that group:
...
5. All fungi EXCEPT Helotiales

```{r}
library("rentrez")

query1 <- "(Helotiales[ORGN] OR Sclerotinia[ORGN] OR Phialocephala[ORGN] OR Marssonina[ORGN] 
...
query2 <- "(Lachnum[ORGN] OR Lachnellula[ORGN] AND (rRNA[Feature] OR misc_RNA[Feature])) 
            NOT(environmental samples[organism] OR metagenomes[orgn] OR unidentified[orgn])"
query3 <- "(Lachnellula[ORGN] AND (rRNA[Feature] OR misc_RNA[Feature])) 
            NOT(environmental samples[organism] OR metagenomes[orgn] OR unidentified[orgn])"
query4 <- "(Helotiales[ORGN] AND (rRNA[Feature] OR misc_RNA[Feature])) 
            NOT(environmental samples[organism] OR metagenomes[orgn] OR unidentified[orgn]
            OR Lachnellula[ORGN])"
query5 <- "(Ustilaginomycetes[ORGN] AND (rRNA[gene] OR 26S[gene] OR ribosomal[product]) 
            NOT(environmental samples[organism] OR metagenomes[orgn] OR unidentified[orgn])"
query6 <- "(Lachnellula[ORGN] AND (rRNA[Feature] OR misc_RNA[Feature])) 
            NOT(Lachnellula willkommii[ORGN] OR environmental samples[organism] OR metagenomes[orgn]
            OR unidentified[orgn])"
```

#### Generate the databases as gene ID lists, *gilist.txt*, and write these to the GenBank folder:
```{r}

webEnvSearch1 <- entrez_search(db="nuccore", query1, retmax=999999)
webEnvSearch2 <- entrez_search(db="nuccore", query2, retmax=999999)
webEnvSearch3 <- entrez_search(db="nuccore", query3, retmax=999999)
webEnvSearch4 <- entrez_search(db="nuccore", query4, retmax=999999)
webEnvSearch5 <- entrez_search(db="nuccore", query5, retmax=999999)
webEnvSearch6 <- entrez_search(db="nuccore", query6, retmax=999999)

length(webEnvSearch1$ids)
length(webEnvSearch2$ids)
length(webEnvSearch3$ids)
length(webEnvSearch4$ids)
length(webEnvSearch5$ids)
length(webEnvSearch6$ids)

write.table(webEnvSearch1$ids, file = paste(genBankPath, "query1_gilist.txt", sep = ""), 
            append = FALSE, quote  = FALSE, row.names = FALSE, col.names = FALSE)
write.table(webEnvSearch2$ids, file = paste(genBankPath, "query2_gilist.txt", sep = ""), 
            append = FALSE, quote  = FALSE, row.names = FALSE, col.names = FALSE)
write.table(webEnvSearch3$ids, file = paste(genBankPath, "query3_gilist.txt", sep = ""), 
            append = FALSE, quote  = FALSE, row.names = FALSE, col.names = FALSE)
write.table(webEnvSearch4$ids, file = paste(genBankPath, "query4_gilist.txt", sep = ""), 
            append = FALSE, quote  = FALSE, row.names = FALSE, col.names = FALSE)
write.table(webEnvSearch5$ids, file = paste(genBankPath, "query5_gilist.txt", sep = ""), 
            append = FALSE, quote  = FALSE, row.names = FALSE, col.names = FALSE)
write.table(webEnvSearch6$ids, file = paste(genBankPath, "query6_gilist.txt", sep = ""), 
            append = FALSE, quote  = FALSE, row.names = FALSE, col.names = FALSE)
```
...
```{r}
giLists <- list.files(path = paste(genBankPath, sep = ""), 
                      pattern   = "\\gilist.txt$", 
                      recursive = FALSE)
metadata <- data.frame(giLists)
metadata$queryDataBase <- sub("_gilist.txt", "", metadata$giLists, ignore.case = FALSE)
...
cmd <- with(metadata,
            paste(pathBlastn,
                  ...
                  " -gilist ", paste(genBankPath, giLists, sep = ""),
                  " -gapopen ", gapOpen,
                  " -gapextend ", gapExtend,
                  " -xdrop_gap ", xDropGap,
                  ...
                  " ' -out ", paste(genBankPath, queryDataBase, "_GenBank.fasta.out", 
                                    sep = ""),
                  sep = ""))
...
#### Record the name of the blast output files in the metadata table:
```{r}
for(k in 1:nrow(metadata)){
    metadata$BlastGenBank_output <- paste(metadata$queryDataBase, "_GenBank.fasta.out", sep = "")
}
```
...
# for(k in 1:nrow(metadata)){
#     gbBlastaTable[k] <- read.table(paste(genBankPath, metadata$BlastGenBank_output, sep = ""),
#                                    header = FALSE,
...
gbBlastTable <- read.table(paste(genBankPath, metadata$BlastGenBank_output[i], sep = ""),
                           header = FALSE,
                           stringsAsFactors = FALSE)
...
write.xlsx(dataset2, file = paste(sharedPath, idFolder, 
                                  metadata$queryDataBase[i], "_to_check_BLAST_results_table.xlsx", 
                                  sep = ""), 
           sheetName = "Sheet1", 
           col.names = TRUE, 
           row.names = TRUE, 
...
*Note: I did not finish the following chunk for query4, I lost all my sequences with the following settings for that one.*
```{r}

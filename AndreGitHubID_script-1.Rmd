...
```{r}
RemoveQsubTempFiles(sharedPathAn, prefix) # This is no longer working...
#idFastaFiles <- list.files(path = paste(sharedPathAn, prefix, sep = ""), 
                           pattern = "\\.sh$|\\.sub$", 
                           recursive = FALSE)
```
...
queryMetadata$NewGBTable <- paste(queryMetadata$BlastGB_FastaOUT, ".New.txt", sep = "")
...
for (i in 1:nrow(queryMetadata)) {
    ...
    gbBlastTableTemp <- read.table(paste(genBankPath, queryMetadata$NewGBTable[i], sep = ""),
                                   sep = "\t", header = TRUE, comment.char = "", quote = "",
                                   as.is = TRUE)
    gbBlastTableTemp$orientation <- sign(gbBlastTableTemp$s_end - gbBlastTableTemp$s_start)
    gbBlastTableTemp$length      <- abs(gbBlastTableTemp$s_end - gbBlastTableTemp$s_start)
    gbBlastTableTemp$query_id    <- as.character(gbBlastTableTemp$query_id)
    
    # Put together data from multiple hits on single line
    dataAgg <- aggregate(gbBlastTableTemp[c(2:14)], 
                         by  = list(gbBlastTableTemp$GB_accession, gbBlastTableTemp$GB_accession),
                         FUN = c)
    
    # find min and max and sense of hits
    for(h in 1:nrow(dataAgg)) {
        dataAgg$min[h]   <- min(unlist(c(dataAgg$s_start[h],dataAgg$s_end[h])))
        dataAgg$max[h]   <- max(unlist(c(dataAgg$s_start[h],dataAgg$s_end[h])))
        dataAgg$sense[h] <- mean(unlist(c(dataAgg$orientation[h])))
        }
    ...
    write.table(checkOrder, 
                file = paste(sharedPath, idFolder, 
                         queryMetadata$QueryName[i], "_CHECK_ORDER_OF_FASTA_file_AND_BLAST_TABLE.csv", 
                         sep = ""), 
                append = FALSE, 
                sep = ",", 
                col.names = NA)
    ...
    write.table(dataAgg,
                file = paste(sharedPath, idFolder, 
                             queryMetadata$QueryName[i], "_dataAgg.txt", sep = ""),
                sep = "\t",
                append = FALSE, quote  = FALSE, row.names = FALSE, col.names = TRUE)
    attributes(gbDNAstring)
    write.dna(gbDNAstring, 
                     file = paste(genBankPath, queryMetadata$QueryName[i], "_gbDNAstring.fasta", 
                                  sep = ""), 
                     format = "fasta", 
                     append = FALSE)
}
queryMetadata$CheckOrder_FastaBlast <- paste(queryMetadata$QueryName, 
                                             "_CHECK_ORDER_OF_FASTA_file_AND_BLAST_TABLE.csv",
                                             sep = "")
queryMetadata$CheckBlastResults     <- paste(queryMetadata$QueryName, 
                                             "_to_check_BLAST_results_table.xlsx",
                                             sep = "")
queryMetadata$dataAgg     <- paste(queryMetadata$QueryName, "_dataAgg.txt", sep = "")
queryMetadata$gbDNAstring <- paste(queryMetadata$QueryName, "_gbDNAstring.fasta", sep = "")
```
*** Will not loop into the next section 
```{r}

...
```{r}
cmd <- paste(" cat ", paste(sharedPath, idFolder, as.character(idFastaFiles), collapse = " ", sep = ""),
...
align <- read.dna(paste(sharedPath, idFolder, "query_aligned_fasta.fasta", sep = ""), format = "fasta")
...
gbBlastTable <- read.table(paste(genBankPath, "fasta_for_GenBank3.fasta.out", sep = ""),
                             header = FALSE, 
...
write.table(checkOrder, 
            file = paste(sharedPath, idFolder, "CHECK ORDER OF FASTA file AND BLAST TABLE_3.csv", 
                         sep = ""), 
            append = FALSE, 
            sep = ",", 
            col.names = NA)
...
cmd2 <- paste("cat ", # fastaForGenBankPath, " ",
...
align1 <- readAAStringSet(alignmentFile2, format = "fasta")
...
# Code below doesn't work, looks like it's hanging, where is the input?
...
Now I have the aligned sequences and the newick tree. I can bring these into Geneious to visualize. 
I had excluded my Lachnellula willkommii ITS concensus sequence that I obtained using the ITSx 
fungal extractor from when generating the trimmed file and tree because it was so long that I 
thought it would be problematic. So in Geneious I extracted the sequences as a list from the 
alignment file "All_files_aligned.fasta". I then made a new list and grouped the ITS concensus
sequence I had generated with the list of extracted sequences and exported that as a fasta file
called "All_files_aligned_extraction_210_SequencesGeneious". I want to go through the script again 
and do the trimming and tree properly now that I have all the sequences. Sequences I don't see 
captured in the blast hits in the initial GenBank search are:

Glarea lozoyensis (Under Helotiales order? - yes, anamorph)
Marssonina brunnea (Under Helotiales order? - yes, combination name type, Marssonina=female, male=Gloeosporium brunneum)
Phialocephala subalpina (although other Phialocephala present)(Under order Helotiales? yes, Basionym, feminine)
Sclerotinia borealis (Under Helotiales? yes, Basionym, Feminine)

For my mitochondrial annotation and comparison I had used these a s references - I feel that they
should be in the alignments and tree as well. I will check to see if this works out.

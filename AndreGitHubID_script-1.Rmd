...
```{r}
querySingleFasta <- "Visaacii_tRNA-L-G-Asp-Ser.fasta"
```
...
```{r}
names(queryFasta) <- "Visaacii_tRNA-L-G-Asp-Ser"
```
...
queryCsv <- list.files(path = paste(sharedPath, idFolder, sep = ""), 
                       ...
                       recursive = FALSE)
queryCsv
queryCsv <- queryCsv[1] # Choose the correct csv file number from those printed out in the console
queryCsv
...
Important: Look at the output files from the previous qsub run, namely the files with the 
suffix "_GenBank.fasta.out".
...
epty
metaquery <- sub(genBankPath, "", epty)
metaquery <- sub("_GenBank.fasta.out", "", metaquery)
unlink(epty, recursive = TRUE, force = FALSE)
queryMetadataOld <- queryMetadata
queryMetadata <- queryMetadata[!(queryMetadata$QueryName==metaquery),] #d<-d[!(d$A=="B" & d$E==0),]
queryMetadata <- subset(queryMetadata, (!queryMetadata$QueryName==metaquery))
row.names(queryMetadata) = NULL
```


*** I ran this clean 2 weeks ago, not sure if I changed something by accident,
not all uniqueGB.txt files are being written
...
*** Over here!!! The problem is now that my loop only uses the first read.GenBank2 file.

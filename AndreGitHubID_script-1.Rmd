...
```{r}
querySingleFasta <- "with_mfannot_added-contig00001-cox1_gene-portion_overlapped_by_bos_taurus_mtco1.fasta"
```
...
```{r}
names(queryFasta) <- "Lawii_COX1_partial_region"
```
...
***Over here!!! For the chunk below, can't run chunk clean as there are some query blasts that 
return no no hit for Cox1 in my query set.
...
# ***Over here!! how do I fix the Ape package to do https (instead of http)?
